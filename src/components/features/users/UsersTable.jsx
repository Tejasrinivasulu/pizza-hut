...
const UsersTable = ({ users }) => {
    return (<Table aria-label="Users Table" isStriped classNames={{ th: "text-md", td: "text-md text-gray-300" }}>
      <TableHeader>
      ...
      {users.length > 0 ? (<TableBody>
          {users.map((user) => (<TableRow key={user._id}>
              <TableCell>
                <UserUI avatarProps={{ src: user.image, size: 'lg', showFallback: true }} description={(<Link href="" size="sm" className="text-blue-500">
                      {user.email}
                    </Link>)} name={user.name} classNames={{ name: "text-md", description: "text-sm" }}/>
              </TableCell>
